...
class Emissions:
    # constructor
    def __init__(self,name,nameOutput,pollutants):
...
################################    
    ...
    def readLineSources(self):
        ...
        if self.LineSources.empty==False:
            ...
            self.__checkColumnsInput(self.ExpectedColumnsLineSources,self.LineSources.columns,'LineSources')
    ...
    def readPointSources(self):
        ...
        if self.PointSources.empty==False:
            ...
            self.__checkColumnsInput(self.ExpectedColumnsPointSources,self.PointSources.columns,'PointSources')
    ...
    def readSurfaceSources(self):
        ...
        if self.SurfaceSources.empty==False:
            self.__checkColumnsInput(self.ExpectedColumnsSurfaceSources,self.SurfaceSources.columns,'SurfaceSources')
    ...
    def readAltSurfaceSources(self):
        ...
        if self.AltSurfaceSources.empty==False:
            self.AltSurfaceSources(self.ExpectedColumnsAltSurfaceSources,self.AltSurfaceSources.columns,'AltSurfaceSources')
...
################################       
    ...
    def __checkColumnsInput(self,ExpectedColumns,ReceivedColumns,type):
        check=True
        if  not(len(ReceivedColumns)==len(ExpectedColumns)):
            check=False
            
        elif not((ReceivedColumns==ExpectedColumns).all()):
            check=False
        
        if check:
            print 'Input Emissions OK'
        else:
            print 'Columns of '+type+' file are not correct'
            print "I will continue, don't blame me if things go wrong"
                
    # make the expected column input              
    def __ExpectedColumnsInput(self):
        temp=['Lat1','Lon1','Lat2','Lon2','Width (m)','Height (m)','Spread (m)']
        temp.extend(self.Pollutants)
        temp.extend(['Wind/Temp dependence','Time factor','NO/NOx ratio','Type','Sector'])
        self.ExpectedColumnsLineSources=temp 
           
        temp=['Lat','Lon','Height (m)','Diameter (m)','Temperature (K)','Volume stream (Nm3/s)']
        temp.extend(self.Pollutants)
        temp.extend(['Wind/Temp dependence','Time factor','NO/NOx ratio','Type','Sector'])
        self.ExpectedColumnsPointSources=temp
        
        temp=['Lat1','Lon1','Lat2','Lon2']
        temp.extend(self.Pollutants)
        temp.extend(['Height (m)','Spread (m)','Wind/Temp dependence','Time factor','NO/NOx ratio','Type','Sector'])
        self.ExpectedColumnsSurfaceSources=temp
        
        temp=['Lat','Lon','Area (m2)']
        temp.extend(self.Pollutants)
        temp.extend(['Height (m)','Spread (m)','Wind/Temp dependence','Time factor','NO/NOx ratio','Type','Sector'])
        self.ExpectedColumnsAltSurfaceSources=temp
