...
def terrain2urbclim(configfile_input):
    ...
    required = ['nx', 'ny', 'clat', 'clon', 'dxy', 'EPSG', 'pname_modis', 'veg_option', 'fname_dem', 'fname_soil',
                'fname_lcz', 'fname_land_parms', 'fname_ahf', 'fname_surface']
    ...
    str3 = 'gdalwarp -s_srs EPSG:32650 -t_srs EPSG:' + params['EPSG'] + ' -te ' + str(xmins) + ' ' + str(ymins) + ' ' + str(
        xmaxs) + ' ' + str(ymaxs) + ' -tr ' + str(dxy) + ' ' + str(dxy) + ' -r mode ' + params[
               'fname_lcz'] +' '+ logfolder+'/temp_lcz.tif'
    execute(str3)

    vtyp = read_tiff(logfolder+'/temp_lcz.tif')
    ...
    with open(params['fname_land_parms'], 'r') as params_file:
        for line in params_file:
            if len(line.strip()) <> 0 and line.strip()[0] != '#':
                ...
                soil1.append(float(splitted[9]))
                ahf1.append(float(splitted[10]))
                urbclim1.append(float(splitted[11]))

    if count <> nroftypes:
        raise Exception('Could not find input data for all China land use classes, check your land_parameters.dat file!')

    # Create gridded input of UrbClim parameters
    # -----------------------------------------------------
    albv = np.full_like(lan, 0)
    albs = np.full_like(lan, 0)
    epsv = np.full_like(lan, 0)
    epss = np.full_like(lan, 0)
    z0m = np.full_like(lan, 0)
    rst = np.full_like(lan, 0)
    rp = np.full_like(lan, 0)
    rootb = np.full_like(lan, 0)
    soil = np.full_like(lan, 0)
    ahf = np.full_like(lan, 0)
    urbclim = np.full_like(lan, 0)

    for i in range(25):
      albv[np.where(lan == lcz1[i])]=albv1[i]
      albs[np.where(lan == lcz1[i])]=albs1[i]
      epsv[np.where(lan == lcz1[i])]=epsv1[i]
      epss[np.where(lan == lcz1[i])]=epss1[i]
      z0m[np.where(lan == lcz1[i])]=z0m1[i]
      rst[np.where(lan == lcz1[i])]=rst1[i]
      rp[np.where(lan == lcz1[i])]=rp1[i]
      rootb[np.where(lan == lcz1[i])]=rootb1[i]
      soil[np.where(lan == lcz1[i])]=soil1[i]
      ahf[np.where(lan == lcz1[i])]=ahf1[i]
      urbclim[np.where(lan == lcz1[i])]=urbclim1[i]

    landmask = np.full_like(ahf, 0)
    landmask[np.where(urbclim < 13)] = 1

    seemask = np.full_like(ahf, 0)
    seemask[np.where(urbclim >= 13)] = 1
    ...
    # ******************************************************************
    # ******************************************************************
    # MODIS monthly mean NDVI (MYD13A3)

    cover = []

    for ii in range(1, 13):
        month = str(ii).zfill(2)
        vgtfile = 'HDF4_EOS:EOS_GRID:"MYD13A3.h26v04.' + month + '.hdf":MOD_Grid_monthly_1km_VI:"1 km monthly NDVI"'
        str4 = 'gdalwarp -t_srs EPSG:' + params['EPSG'] + ' -te ' + str(xmins) + ' ' + str(
            ymins) + ' ' + str(xmaxs) + ' ' + str(ymaxs) + ' -tr ' + str(dxy) + ' ' + str(
            dxy) + ' ' + vgtfile + ' -r bilinear '+logfolder+'/temp_veg_' + month + '.tif'
        execute(str4)
        dvi = read_tiff(logfolder+'/temp_veg_' + month + '.tif')
        
        vgtfile1 = 'HDF4_EOS:EOS_GRID:"MYD13A3.h26v05.' + month + '.hdf":MOD_Grid_monthly_1km_VI:"1 km monthly NDVI"'
        str41 = 'gdalwarp -t_srs EPSG:' + params['EPSG'] + ' -te ' + str(xmins) + ' ' + str(
            ymins) + ' ' + str(xmaxs) + ' ' + str(ymaxs) + ' -tr ' + str(dxy) + ' ' + str(
            dxy) + ' ' + vgtfile1 + ' -r bilinear '+logfolder+'/temp1_veg_' + month + '.tif'
        execute(str41)
        dvi1 = read_tiff(logfolder+'/temp1_veg_' + month + '.tif')
        
        problem = np.where(dvi < 0)
        dvi[problem]=dvi1[problem]
    ...
    #******************************************************************
    #******************************************************************
    # For now, don't use spatial explicit SST, take the ERA-Interim values if it is needed
    #
    sst= []
    ...
    im = axarr[0, 1].imshow(urbclim, cmap=cmap, norm=norm, extent=[lnmn, lnmx, ltmn, ltmx], aspect='auto')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='terrain2urbclim.py')
    parser.add_argument('configfile', metavar='configfile', type=str, help='Configutation file name')
    args = parser.parse_args()

    
    terrain2urbclim(args.configfile)
