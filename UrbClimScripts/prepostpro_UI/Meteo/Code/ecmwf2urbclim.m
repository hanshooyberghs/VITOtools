function ecmwf2urbclim(filenames,coordinates,data,outputfile,logfolder,detailed_input)
%
...
% Input variables:
%     filenames:     MATLAB structure with inputfiles:
%         filenames.file_2d
%         filenames.file_2d
%     coordinates:        MATLAB structure, containing
%         coordinates.longitude
%         coordinates.latitude
%     data:     MATLAB structure, containing start- and enddate
%         data.startdate
%         data.enddate
%     outputfile:         file waarin data wordt opgeslagen
%     logfolder:          folder waaring figuren worden opgeslagen
%     detailed_input:     MATLAB structure with detailed parameters:
%         detailed_input.timeresolution: time resolution of output in s (typically 1200)
%         detailed_input.etas: soil parameter etas
%         detailed_input.psis: soil parameter psis
%         detailed_input.bch:  soil parameter bch
%         detailed_input.file_modellevels: location of file with modellevels (should be saved in Matlab format)
...
%% process input
% modellevels file
    modellevels_file=detailed_input.file_modellevels;
    
% requested vertical levels    
    zout=[10.,35.,70.,120.,175.,240.,320.,410.,520.,650.,800.,1000.,1250.,1500.,1750.,2000.,2250.,2500.,2750.,3000];
    zout=zout(1:detailed_input.nr_vertlevels);
% time resolution for the meteo input in seconds (see config file)
    tmet=detailed_input.timeresolution;
    
%  soil texture parameters (see config file)
    etas = detailed_input.etas;
    psis = detailed_input.psis;
    bch = detailed_input.bch;
...
% start and enddate
    startdate=floor(data.startdate)+3/24;   % always start at 3 o'clock in the morning for consistency
    enddate=floor(data.enddate)+1;            % always end at 24 o'clock midnight     
    
% coordinates
    clon=coordinates.longitude;
    clat=coordinates.latitude;
    
% filenames
    inputfile2d=filenames.file_2d;
    inputfile3d=filenames.file_3d; 
...
startdate
enddate
where=find((startdate<=matlabtime) & (matlabtime<= enddate));
if numel(where)==0
    err = MException('InputError:TimeSelection', ...
        'No timesteps between selected start- and enddate.');
    throw(err)  
end

matlabtime=matlabtime(where);
...
datetick('x','mm-YYYY','keeplimits')
ylabel('Rs')
print(f,'-dpng',strcat(logfolder,'ECMWF_timeseries_matlab1.png'));
close(f)
...
datetick('x','mm-YYYY','keeplimits')
ylabel('ps')
print(f,'-dpng',strcat(logfolder,'ECMWF_timeseries_matlab2.png'));
close(f)
...
datetick('x','mm-YYYY','keeplimits')
ylabel('rain')
print(f,'-dpng',strcat(logfolder,'ECMWF_timeseries_matlab3.png'));
close(f)
...
ylabel('Wind surface')
datetick('x','mm-YYYY','keeplimits')
print(f,'-dpng',strcat(logfolder,'ECMWF_timeseries_matlab4.png'));
close(f)
...
print(f,'-dpng','-r300',strcat(logfolder,'ECMWF_vertical_profiles_matlab.png'));
close(f)

% enkele verticale potentiele temperatuursprofielen
f=figure('visible','off');
hold on
set(f,'Position',[0,0,1000,1000]);
...
for i=1:8
    subplot(2,4,i)
    moment=28+(i-1)*36;
    plot(Ptout(:,moment),zout);
    axis([min(Ptout(:,moment)) max(Ptout(:,moment)) 0 2500]);
    title([num2str(day(moment)),'/',num2str(month(moment)),'/',num2str(year(moment)),' T',num2str(hour(moment)),'h']);  
end
print(f,'-dpng','-r300',strcat(logfolder,'ECMWF_vertical_profiles_pottemp_matlab.png'));
close(f)
